// 서학동성당 홈페이지 — 관리 기능 API (Cloudflare Pages Functions)
//
...
// 처리된다. 저장은 GitHub 저장소에 직접 커밋해서, Cloudflare Pages가 자동으로 다시 배포한다.
//
...
// 선택:
//   GITHUB_REPO   기본값 jangsangyun0310-sketch/SEOHAKDONG-CATHOLIC-CHURCH
//   GITHUB_BRANCH 기본값 main
//   OWNER_EMAIL   기본값 jangsangyun0310@gmail.com (관리자 명단이 비어 있어도 항상 관리자)

const DEFAULTS = {
  GITHUB_REPO: 'jangsangyun0310-sketch/SEOHAKDONG-CATHOLIC-CHURCH',
  GITHUB_BRANCH: 'main',
  OWNER_EMAIL: 'jangsangyun0310@gmail.com',
  FIREBASE_PROJECT_ID: 'seohakdong-church',
  // push-config.js와 같은 값. 비밀키가 아니라 공개되어도 되는 값이다.
  FIREBASE_API_KEY: 'AIzaSyBFqfixoo2Rky7VU7-WPZ43qisL3dFMY9g',
};
...
function safeImagePath(v) {
  const s = str(v, 300);
  if (!s) return '';
  // 우리 저장소 안의 이미지 경로만 허용
  if (!/^assets\/img\/[A-Za-z0-9_\-./]+\.(jpe?g|png|webp|gif)$/i.test(s) || s.includes('..')) return '';
  return s;
}
  ...
  if (name === 'notices') {
    return {
      items: items.slice(0, 300).map((n) => ({
        tag: str(n.tag, 30), title: str(n.title, 120), date: str(n.date, 20), body: str(n.body, 5000),
      })),
    };
  }
  if (name === 'bulletins') {
...
function safeUploadPath(p) {
  ...
  if (!/^assets\/img\/uploads\/[0-9]{4}\/[A-Za-z0-9_\-]+\.(jpg|png|webp)$/.test(s)) return '';
  return s;
}
...
async function sendPush(env, origin, title, body) {
    ...
    body: JSON.stringify({
      message: {
        ...
        webpush: {
          ...
          notification: { icon: `${origin}/assets/img/icons/icon-192.png`, tag: announcementId },
        },
      },
    }),
  });
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (route === 'save' && request.method === 'POST') {
      ...
      const label = { notices: '공지사항', bulletins: '주보', gallery: '갤러리', announce: '접속 시 팝업' }[name];
